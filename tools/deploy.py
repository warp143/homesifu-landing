...
def main():
    ...
    run_cmd("ssh -i config/homesifu-serverstatus_key.pem azureuser@172.188.96.148 'docker stop homesifu-website 2>/dev/null || true'")
    run_cmd("ssh -i config/homesifu-serverstatus_key.pem azureuser@172.188.96.148 'docker rm homesifu-website 2>/dev/null || true'")
    
    # Create nginx container with port mapping (no custom nginx config needed)
    print("🔧 Creating new container with port 8081...")
    run_cmd("ssh -i config/homesifu-serverstatus_key.pem azureuser@172.188.96.148 'docker run -d --name homesifu-website -p 8081:80 --restart unless-stopped nginx:alpine'")
    
    # Copy files into container
    print("📁 Copying files into container...")
    run_cmd("ssh -i config/homesifu-serverstatus_key.pem azureuser@172.188.96.148 'docker cp /home/azureuser/homesifu-landing/. homesifu-website:/usr/share/nginx/html/'")
    
    # Check status
    print("📊 Checking container status...")
    run_cmd("ssh -i config/homesifu-serverstatus_key.pem azureuser@172.188.96.148 'docker ps | grep homesifu-website'")
